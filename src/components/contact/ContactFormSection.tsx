...
export default function ContactFormSection({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/booking-inquiry-form", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(responseData.error || "Failed to send booking inquiry");
      }
      ...
      onFormSubmit(
        "success",
        "Your booking inquiry has been sent successfully! We will get in touch with you shortly."
      );
    } catch (error) {
  ...
  return (
    <motion.section id="booking-form" ref={ref} className="py-16">
      <motion.div
      ...
      >
        ...
        <div className="relative z-10">
          <h2 className="text-3xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-light via-light/90 to-light/80">
            Booking Inquiry
          </h2>
          <div className="h-1 w-20 bg-gradient-to-r from-primary to-accent rounded-full mb-6"></div>

          <p className="text-light/80 mb-8">
            Interested in booking Towizaza for a show, festival, or private
            event? Fill out the form below with your event details.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              >
                <span
                  className={`flex items-center gap-2 ${isSubmitting ? "opacity-0" : "opacity-100"} transition-opacity`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
