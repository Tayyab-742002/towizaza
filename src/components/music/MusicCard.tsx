...
export default function MusicCard({ album, variant = 'default', className = '' }: MusicCardProps) {
  ...
  const cardClasses = {
    default: "bg-dark/50 backdrop-blur-lg rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300",
    compact: "bg-dark/50 backdrop-blur-lg rounded-lg overflow-hidden",
    featured: "bg-gradient-to-br from-dark/80 to-secondary/30 backdrop-blur-lg rounded-lg overflow-hidden shadow-lg"
  };
  ...
  return (
    ...
    >
      <div className={imageContainerClasses[variant]}>
        ...
        >
          {hasPlayableTracks && (
            <button 
              ...
              aria-label={isCurrentAlbum && state.isPlaying ? "Pause" : "Play Album"}
            >
              {isCurrentAlbum && state.isPlaying ? (
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-light">
        ...
        {/* Upcoming badge */}
        {album.upcoming && (
          <div className="absolute bottom-2 left-2 bg-accent text-light text-xs font-medium py-1 px-2 rounded-full">
            Coming Soon
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        <Link href={getAlbumUrl()} className="group">
          <h3 className="text-lg font-medium text-light group-hover:text-primary transition-colors duration-200">{album.title}</h3>
