...
export default function BiographySection({ artistInfo }: BiographySectionProps) {
  ...
  const getAltImageUrl = () => {
    if (!artistInfo?.altImage) return '/images/artist-alt-placeholder.jpg';
    
    try {
      if (typeof artistInfo.altImage === 'string') {
        return artistInfo.altImage;
      }
      
      // If it's a Sanity image
      return urlFor(artistInfo.altImage).url();
    } catch (error) {
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="text-light/60 text-center max-w-xl">
            From beginnings to breakthroughs, explore the artistic evolution that defines Towizaza's unique sound.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          >
            <div className="relative h-[500px] w-full overflow-hidden rounded-2xl">
              ...
              <div className="absolute top-4 right-4 bg-dark/60 backdrop-blur-md border border-light/10 py-1 px-4 rounded-full">
                <span className="text-sm font-medium text-light/90">Est. {artistInfo?.foundingYear || "2018"}</span>
              </div>
            </div>
            ...
            <div className="absolute -bottom-6 left-6 right-6 bg-dark/80 backdrop-blur-lg border border-light/10 p-5 rounded-xl">
              ...
              >
                ...
                <p className="italic text-light/90 text-lg leading-relaxed pl-6">
                  {artistInfo?.quote || "Music is not just what I do—it's the lens through which I experience the world."}
                </p>
              </motion.blockquote>
            </div>
          </motion.div>
          ...
          >
            <motion.p 
              ...
              variants={item}
            >
              {artistInfo?.bio?.part1 || "Born in the digital age yet nostalgic for analog warmth, Towizaza emerged from the fusion of classical training and electronic experimentation. With roots in both traditional composition and cutting-edge production techniques, the artist crafts soundscapes that defy conventional genre boundaries."}
            </motion.p>
            ...
            <motion.p 
              ...
              variants={item}
            >
              {artistInfo?.bio?.part2 || "Since debuting in 2018, Towizaza has pushed the boundaries of sonic innovation while maintaining an emotional core that resonates with listeners worldwide. The music combines intricate layering with minimalist restraint, creating signature sounds that feel both familiar and revolutionary."}
            </motion.p>
            ...
            >
              <p className="text-light/80 text-lg">
                {artistInfo?.bio?.part3 || "With each release, the project evolves, incorporating new influences while staying true to a distinctive aesthetic that fans have come to recognize and love. Collaborations with artists across genres have further expanded the sonic palette, creating unexpected fusions that challenge expectations."}
              </p>
            </motion.div>
            ...
            >
              {/* Achievement badges */}
              {[
                { label: 'Albums Released', value: artistInfo?.stats?.albumsReleased || 5 },
                { label: 'Countries Toured', value: artistInfo?.stats?.countriesVisited || 15 },
                { label: 'Collaborations', value: artistInfo?.stats?.collaborations || 12 }
              ].map((stat, index) => (
                <div 
                  key={index}
