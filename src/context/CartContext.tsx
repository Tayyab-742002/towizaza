  ...
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      const existingItemIndex = state.items.findIndex(
        (item) =>
          generateCartItemId(item.product, item.variant, item.size) ===
          cartItemId
      );
      ...
      if (existingItemIndex > -1) {
        ...
        return {
          ...
          items: [
            ...state.items,
            { id: cartItemId, product, quantity, variant, size },
          ],
        };
      }
    }
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // Adding a flag to avoid updating during an ongoing render
      ...
      try {
        ...
        if (savedCart) {
          ...
          if (parsedCart.items && Array.isArray(parsedCart.items)) {
            // Use a timeout to ensure we're not updating during render
            setTimeout(() => {
              // Only proceed if we're still mounting/loading
              if (isInitialLoad) {
  ...
  const addToCart = (
    ...
    dispatch({
      type: "ADD_ITEM",
      payload: { product, quantity, variant, size },
    });
  };
