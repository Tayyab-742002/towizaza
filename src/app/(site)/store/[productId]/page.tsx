      ...
      try {
        ...
        if (data) {
          ...
          if (data.sizes && data.sizes.length > 0) {
            const firstInStockSize = data.sizes.find(
              (size: ProductSize) => size.inStock
            );
            if (firstInStockSize) {
          ...
          if (data.variants && data.variants.length > 0) {
            const firstInStockVariant = data.variants.find(
              (variant: ProductVariant) => variant.inStock
            );
          ...
          if (fallbackProduct) {
            ...
            if (fallbackProduct.sizes && fallbackProduct.sizes.length > 0) {
              const firstInStockSize = fallbackProduct.sizes.find(
                (size) => size.inStock
              );
              if (firstInStockSize) {
                setSelectedSize(firstInStockSize.id);
            ...
            if (
              ...
              fallbackProduct.variants.length > 0
            ) {
              const firstInStockVariant = fallbackProduct.variants.find(
            ...
            if (
              fallbackProduct.relatedProducts &&
              fallbackProduct.relatedProducts.length > 0
            ) {
    ...
    <div className="min-h-screen bg-dark text-light">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-16 sm:py-24 relative z-10">
        ...
        >
          <Link
            href="/store"
            className="hover:text-light transition-colors hover:underline"
          >
            Store
          </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 sm:gap-14">
          ...
          >
            ...
            <motion.div className="mb-4 sm:mb-6" variants={itemVariants}>
              ...
              {!product.inStock && (
                <span className="ml-4 px-3 py-1 bg-primary/20 text-primary rounded-full text-sm">
                  Out of Stock
                </span>
              )}
            </motion.div>
            ...
            >
              {product.featured && (
                <span className="px-3 py-1 bg-accent/20 backdrop-blur-sm text-accent rounded-full text-xs font-medium">
                  Featured
                </span>
              )}
              {product.new && (
                <span className="px-3 py-1 bg-primary/20 backdrop-blur-sm text-primary rounded-full text-xs font-medium">
                  New
                </span>
              )}
        ...
        {relatedProducts.length > 0 && (
          <motion.div
            ...
            animate={
              relatedInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 40 }
            }
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
              {relatedProducts.map((related: any, index: number) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 20 }}
                  animate={
                    relatedInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }
                  }
                    ...
                    <div className="aspect-square bg-gradient-to-br from-dark/80 to-mid/20 p-1 rounded-xl overflow-hidden mb-4 shadow-lg">
                      <div className="w-full h-full rounded-lg overflow-hidden">
                        {related.images && related.images.length > 0 && (
                          <img
                            src={
                              typeof related.images[0] === "string"
                                ? related.images[0]
