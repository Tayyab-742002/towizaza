...
export default function MusicPage() {
  ...
  const processedMusic = musicData
    ...
    .sort((a, b) => {
      if (sortBy === 'newest') {
        return new Date(b.releaseDate || b.year).getTime() - new Date(a.releaseDate || a.year).getTime();
      } else if (sortBy === 'oldest') {
        return new Date(a.releaseDate || a.year).getTime() - new Date(b.releaseDate || b.year).getTime();
      } else { // a-z
      ...
      >
        ...
        <div className="container mx-auto px-6 relative z-10">
          <motion.div
            ...
            className="max-w-4xl"
          >
            <h1 className="text-6xl md:text-7xl font-bold mb-4">Music Catalog</h1>
            <p className="text-xl text-light/80 max-w-2xl">
              Explore Towizaza's discography, including albums, singles, and exclusive releases. Listen to previews and discover the complete catalog.
            </p>
          ...
          >
            <div className="bg-dark/30 backdrop-blur-md px-6 py-3 rounded-lg border border-light/5">
              <div className="text-primary font-bold text-3xl">{musicData.filter(album => album.type === 'album').length}</div>
              <div className="text-light/70">Albums</div>
            </div>
            <div className="bg-dark/30 backdrop-blur-md px-6 py-3 rounded-lg border border-light/5">
              <div className="text-primary font-bold text-3xl">{musicData.filter(album => album.type === 'single').length}</div>
              <div className="text-light/70">Singles</div>
            </div>
            <div className="bg-dark/30 backdrop-blur-md px-6 py-3 rounded-lg border border-light/5">
              <div className="text-primary font-bold text-3xl">{musicData.filter(album => album.type === 'ep').length}</div>
              <div className="text-light/70">EPs</div>
            </div>
            <div className="bg-dark/30 backdrop-blur-md px-6 py-3 rounded-lg border border-light/5">
              <div className="text-primary font-bold text-3xl">
                {musicData.reduce((total, album) => total + album.tracks.length, 0)}
              </div>
              <div className="text-light/70">Total Tracks</div>
            </div>
          </motion.div>
        </div>
      </motion.section>
      ...
      >
        <div className="container mx-auto px-6">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-3xl font-bold">Music <span className="text-primary">Catalog</span></h2>
                ...
                <div className="bg-dark/30 backdrop-blur-md border border-light/10 rounded-xl p-5">
                  <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                    ...
                    <div className="w-full md:w-auto">
                      <label className="block text-light/70 text-sm mb-2">Type</label>
          ...
          <div className="mb-8 text-light/70 text-sm">
            Showing {processedMusic.length} {processedMusic.length === 1 ? 'result' : 'results'}
            ...
            >
              {processedMusic.length > 0 ? (
                ...
                >
                  ...
                  <p className="text-light/60 mb-6">No music matches your search criteria. Try adjusting your filters.</p>
                  <button 
